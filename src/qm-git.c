/* qm-git.c — the shell entry point for mv_git on OpenQM / ScarletDME.
 * Copyright (C) 2026 Gordon Heydon.  GPL-2.0-only (see LICENSE).
 *
 * FIRST PASS (mv_git#243).  QM is the MVX/jBASE case rather than the UniData
 * one: a standalone process makes its own connection with QMConnectLocal()
 * and calls the record API itself (qmgit_rt.c), so this driver runs the shared
 * engine directly.  There is no agent session to keep alive and no daemon.
 *
 * IT NEEDS AN ACCOUNT NAME, WHICH THE OTHER DRIVERS DO NOT.  udt, uv and jsh
 * are entered by being in the account's directory; QMConnectLocal takes the
 * account by name, and outside a session there is no current account to
 * inherit.  So `-a` here means both things at once: the QM account to connect
 * to, and — when a directory of that name is present — the directory to work
 * in.  $QMACCOUNT supplies it when `-a` is absent.
 *
 * Anything the engine does not implement is forwarded to the real git, so a QM
 * account behaves like a git working tree for every non-record command.
 */
#include "mvxgit.h"
...
/* QM account names are upper case in the account register, while their
   directories usually are not (/home/test holds account TEST).  Fold, so
   `-a test`, `-a TEST` and standing in ./test all reach the same account. */
static const char *qm_account_name(const char *s) {
    static char buf[256];
    size_t n = 0;
    if (!s) return "";
    /* A path names its last component: `-a /srv/accounts/sales` is SALES. */
...
static const char *cwd_name(void) {
 ...
 * where the engine does not look.  udt-git and uv-git both do this; qm-git did
...
/* Make the directory a real OpenQM ACCOUNT.
 *
 * CREATE-ACCOUNT gives it a VOC, $HOLD, $SAVEDLISTS and a private catalogue,
 * and — the part that matters and that no amount of mkdir reproduces — adds it
 * to QM's REGISTER OF ACCOUNTS.  QMConnectLocal() takes an account by name and
 * looks it up in that register, so a cloned directory that was never
 * registered cannot be connected to at all: every later qm-git command in it
 * would fail at the connection, not at the record.
 *
 * Unlike jBASE, QM's CREATE-ACCOUNT is happy with a directory that already has
 * files in it (verified on ScarletDME 2.6-6), so a clone's records and .git
 * can already be in place — no temporary directory, no moving furniture.
 *
 * Idempotent: an account that already has a VOC is left alone.
 *
 * Run through the `qm` shell rather than QMClient because there is nothing to
 * connect to yet: the account being created is the one we would need. */
static void qm_make_account(const char *dir) {
    char voc[4200];
    snprintf(voc, sizeof voc, "%s/VOC", dir);
    if (access(voc, F_OK) == 0) return;          /* already an account */

    char abs[PATH_MAX];
    if (!realpath(dir, abs)) {
        fprintf(stderr, "qm-git: cannot resolve %s: %s\n",
                dir, strerror(errno));
        return;
    }
    const char *name = mv_qm_account();
    if (!name || !*name) {
        fprintf(stderr, "qm-git: no account name to register %s under; "
                        "pass -a <account>\n", abs);
        return;
    }
    const char *qm = getenv("QMBIN");
    if (!qm || !*qm) qm = "qm";
    char cmd[9000];
    /* -aQMSYS, NOT BARE.  `qm` with no account uses the CURRENT DIRECTORY,
       and when that is not a registered account it stops to ask "Current
       directory X is not a valid account. Create account?" -- which eats the
       sentence.  We are always standing in the directory we are about to
       register, so bare `qm` hits that prompt every single time. */
    snprintf(cmd, sizeof cmd,
             "'%s' -aQMSYS 'CREATE-ACCOUNT %s %s' </dev/null >/dev/null 2>&1",
             qm, name, abs);
    if (system(cmd) != 0 || access(voc, F_OK) != 0)
        fprintf(stderr,
            "qm-git: could not register %s as QM account %s "
            "(CREATE-ACCOUNT).\n"
            "        It will hold files, but qm-git cannot connect to it "
...
static int ask_open_account(void) {
    ...
    if (!isatty(STDIN_FILENO)) {
        fprintf(stderr, "qm-git: keeping the open account format "
...
static int run_git(int argc, char **argv, int from) {
    ...
    if (!g) { perror("qm-git"); return 1; }
    g[0] = (char *)"git";
    int n = 1;
    for (int i = from; i < argc; i++) g[n++] = argv[i];
    g[n] = NULL;
    pid_t pid = fork();
    if (pid < 0) { perror("qm-git: fork"); free(g); return 1; }
    if (pid == 0) {
        execvp(g[0], g);
        fprintf(stderr, "qm-git: cannot run git: %s\n", strerror(errno));
        _exit(127);
    }
    int st = 0;
...
int main(int argc, char **argv) {
    ...
    if (i >= argc || !strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
        fputs("usage: qm-git [-a <account>] <command> [args]\n"
              "  -a names the QM account (and its directory, if present);\n"
              "     $QMACCOUNT is used when -a is absent\n"
              "  init | add | rm | commit | status | log | diff | show\n"
    ...
    /* Two jobs, deliberately: name the QM account AND enter its directory.
       If `-a` names a directory here, work in it; otherwise assume we are
       already standing in the account and `-a` only says which one to connect
       to.  Getting this wrong is quiet — relative paths would resolve against
       the wrong tree — so an explicit account that is neither is an error. */
    if (strcmp(account, ".") != 0) {
        struct stat st;
        if (stat(account, &st) == 0 && S_ISDIR(st.st_mode)
            && chdir(account) != 0) {
            fprintf(stderr, "qm-git: cannot enter %s: %s\n",
                    account, strerror(errno));
            return 1;
        }
        mv_qm_set_account(qm_account_name(account));
    } else {
        const char *env = getenv("QMACCOUNT");
        if (env && *env) mv_qm_set_account(env);
        else             mv_qm_set_account(qm_account_name(cwd_name()));
    }
    ...
    if (!strcmp(sub, "attr")) {
        fprintf(stderr,
            "qm-git: 'attr' is an in-session verb, not a shell command.\n"
            "        Run it inside a OpenQM session in this account:\n"
    ...
    if (!strcmp(sub, "init"))        out = mv_git_init(ctx, repo);
        ...
        else          out = strdup("usage: qm-git add <file> [id] | -A");
    }
        ...
        /* Take a checkout somebody made with plain git and build the account
           ...
           udt-git and uv-git (mv_git#124, #125) -- a OpenQM account is just a
           directory, so unlike UniData there is nothing to create first. */
        const char *dir = p0 ? p0 : ".";
        if (chdir(dir) != 0) {
            fprintf(stderr, "qm-git adopt: cannot enter %s: %s\n",
                    dir, strerror(errno));
            mv_ctx_destroy(ctx);
            return 1;
        }
        ...
        if (!found) {
            fprintf(stderr,
                "qm-git adopt: %s carries no MV account descriptor "
        ...
        case MV_ADOPT_ASK_ENABLE:
            fprintf(stderr,
                "qm-git adopt: %s is already in the open account format, but "
        ...
        case MV_ADOPT_ASK_CONVERT:
            fprintf(stderr, "qm-git adopt: %s is a native %s account and will "
                            "be converted to a OpenQM one.\n", dir, found);
            if (ask_open_account())
                (void)system("git config mvx.openaccount true");
            break;
        default:
            break;              /* native to OpenQM: nothing to convert or ask */
        }
        ...
        if (mv_git_worktree_stash(captured, sizeof captured, &stashed) != 0) {
            fprintf(stderr, "qm-git adopt: could not read the working tree\n");
            mv_ctx_destroy(ctx);
            return 1;
        }
        if (stashed)
            fprintf(stderr, "qm-git adopt: your uncommitted changes are in the "
        ...
        printf("adopted %s as a OpenQM account\n", dir);
    }
        ...
        if (!p0 || !p1) {
            fprintf(stderr, "usage: qm-git clone <url> <dir> [ref]\n");
            mv_ctx_destroy(ctx);
            return 2;
        }
        /* --no-checkout, exactly as udt-git does: the account is BUILT from
           ...
           then contradict.  qm-git used to call mv_git_clone, which is
           git_clone WITH a checkout and nothing after it, so a "clone" left a
           directory tree that OpenQM opens as UD files because it opens any
           ...
           truth on disk. */
        {
            ...
            if (pid < 0 || waitpid(pid, &st, 0) < 0 ||
                ...
                fprintf(stderr, "qm-git clone: git clone failed\n");
                mv_ctx_destroy(ctx);
                return 1;
            }
        }
        ...
        /* mv_git_clone is git_clone and nothing more: it leaves the OPEN FORM
           ...
           say so in their closing line; qm-git did not, so a "clone" produced a
           directory tree that OpenQM would open as UD files because it opens any
        ...
        if (chdir(p1) != 0) {
            fprintf(stderr, "qm-git clone: cannot enter %s\n", p1);
            mv_ctx_destroy(ctx);
            return 1;
        }
        qm_make_account(".");
        ...
        /* A repository committed in the OPEN form has to be checked out as an
           ...
           asked rather than assumed (#88).  Without this, a OpenQM clone of an
        ...
        if (system("git cat-file -e HEAD:.mv-account >/dev/null 2>&1") == 0) {
            if (ask_open_account()) {
                if (system("git config mvx.openaccount true") != 0)
                    fprintf(stderr, "qm-git clone: could not set "
                                    "mvx.openaccount\n");
                else
                    fprintf(stderr, "qm-git: '%s' was committed in the open "
        ...
        /* Forwarded, this answered `git version 2.43.7` -- qm-git reporting
           stock git's version as its own, which is worse than not answering. */
        char self[64];
        snprintf(self, sizeof self, "qm-git %s", MVXGIT_VERSION);
        ...
        if (strcmp(op, "list") != 0 && !*name) {
            fprintf(stderr, "usage: qm-git tag {name {commit} | -a name "
                            "-m msg | -d name}\n");
            mv_ctx_destroy(ctx);
            return 2;
        }
