...
# A STALE STAGE SHIPS A PROGRAM NOBODY BUILT.  Staging copies over whatever a
# previous run left here, so a source that has since been removed or renamed
# would travel in the package with no line of build output mentioning it.
...
# THE SOURCE TRAVELS; THE COMPILE DOES NOT HAVE TO.  Every program ships, so
# the package is one thing on every platform and a reader finds what they
# expect -- but a platform only builds what it can use.
#
# GIT.RPC hands git-object work to a background process because UniVerse BASIC
# cannot reach libgit2 (GCI is licensed and dead in the Trial Edition), and
# GIT.AGENT is the account-side server it talks to.  jBASE is the MVX case: it
# reaches libgit2 directly, GIT.OBJ's only CALL to GIT.RPC sits inside
# $IFDEF UV and compiles out, and neither program has a job here.
#
# Compiling them anyway would mean renaming their identifiers to dodge jBASE
# reserved words -- churn on sources shared with uv and udt, to build two
# programs that cannot run (mv_git#177).
cat > "$STAGE/mv_git/NOCOMPILE" <<'NC'
# Programs that ship but are not cataloged on jBASE, one per line.
# Generated by build-jbase.sh -- see the comment there for why.
GIT.AGENT
GIT.RPC
NC
