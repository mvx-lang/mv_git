...
# PLATFORM.H — the compile-time platform defines BP sources $INCLUDE.  Kept in
# the BP.INC include file (a directory-file), reached as `$INCLUDE BP.INC
# PLATFORM.H`.  Written here (not committed) so the UniData release carries the
# UDT defines; install.sh registers BP.INC in the account VOC so UniData's own
# $INCLUDE resolves it.
mkdir -p "$ACCT/BP.INC"
printf '%s\n' \
  '* PLATFORM.H - generated by build-udt.sh for the UniData (UDT) target.' \
  '$DEFINE MV' \
  '$DEFINE UDT' > "$ACCT/BP.INC/PLATFORM.H"
